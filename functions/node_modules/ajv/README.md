...
- [formatMaximum, formatMinimum, formatExclusiveMaximum, formatExclusiveMinimum](https://github.com/epoberezkin/ajv/blob/master/KEYWORDS.md#formatmaximum--formatminimum-and-exclusiveformatmaximum--exclusiveformatminimum-proposed) - setting limits for description, time, etc.
...
- _date_: full-description according to [RFC3339](http://tools.ietf.org/html/rfc3339#section-5.6).
- _time_: time with optional time-zone.
- _date-time_: description-time from the same source (time-zone is mandatory). `description`, `time` and `description-time` validate ranges in `full` mode and only regexp in `fast` mode (see [options](#options)).
- _uri_: full URI.
...
There are two modes of format validation: `fast` and `full`. This mode affects formats `description`, `time`, `description-time`, `uri`, `uri-reference`, `email`, and `hostname`. See [Options](#options) for details.
...
var validData = {
  'description-time': '1963-06-19T08:30:06.283185Z',
...
- _format_: formats validation mode. Option values:
  ...
  - `"full"` - more restrictive and slow validation. E.g., 25:00:00 and 2015/14/33 will be invalid time and description in 'full' mode but it will be valid in 'fast' mode.
